...
void OpenNIListener::retrieveTransformations(std_msgs::Header depth_header, Node* node_ptr)
{
  ...
  //cout << "base_frame is " << base_frame << endl;
  //cout << "depth_frame_id is " << depth_header.frame_id << endl;
  ROS_INFO_STREAM("base_frame is " << base_frame);
  ROS_INFO_STREAM("depth_frame_id is " << depth_header.frame_id);
  try{
    tflistener_->waitForTransform(base_frame, depth_frame_id, depth_time, ros::Duration(5)); //change duration to from 0.005 to 3
